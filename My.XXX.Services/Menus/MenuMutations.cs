...
public sealed class MenuMutations(TimeProvider clock)
{
    public async Task<Result> Add(IAccessControlWriteSession session, SaveMenu input, string culture, string userId, CancellationToken cancellationToken = default)
    {
        var validation = ValidateAdd(input);
    ...
    public async Task<Result> Update(IAccessControlWriteSession session, EditMenu input, string culture, string userId, CancellationToken cancellationToken = default)
    {
        var validation = ValidateUpdate(input);
        if (validation.IsFailed) return validation;
    ...
    public async Task<Result> Remove(IAccessControlWriteSession session, List<int> ids, string userId, CancellationToken cancellationToken = default)
    {
        var validation = ValidateRemove(ids);
        if (validation.IsFailed) return validation;
    ...
    public async Task<Result> Move(IAccessControlWriteSession session, MenuSortModel input, string userId, CancellationToken cancellationToken = default)
    {
        var menus = (await session.LoadMenus(cancellationToken));
    ...
    public static Result ValidateAdd(SaveMenu input) =>
        Result.OkIf(!(input == null || string.IsNullOrWhiteSpace(input.DisplayName) || !input.IsAction.HasValue || input.ParentId < 0 || input.Number < 0), MenuErrors.InvalidInput());

    public static Result ValidateUpdate(EditMenu input) =>
        Result.OkIf(!(input == null || input.Id <= 0 || input.ParentId < 0 || input.Number < 0 || !MenuUpdateFields.Valid(input.ClearFields)), MenuErrors.InvalidInput());

    public static Result ValidateRemove(List<int> ids) =>
        Result.OkIf(!(ids == null || ids.Count == 0 || ids.Any(id => id <= 0)), MenuErrors.InvalidInput());
